...
/* gm_shim: forced off. Upstream's Windows path replaces the argc/argv the
 * caller passed in with the *host process's* real command line via
 * GetCommandLineW() -- fine for a standalone ffmpeg.exe, but fatal for an
 * in-process embedding where the host (e.g. the Flutter app) has no
 * ffmpeg-shaped argv of its own and every session would see the same
 * (empty) args regardless of what gm_execute was called with. */
